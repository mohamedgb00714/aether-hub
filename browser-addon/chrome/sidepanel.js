/**
 * aethermsaid hub Browser Addon - Side Panel Script
 */
...
async function init() {
  ...
  // Load pending data
  const stored = await chrome.storage.local.get(['pendingQuestion', 'pendingSummarize']);
...
async function handleConnect() {
  ...
  try {
    await chrome.runtime.sendMessage({ action: 'connect', secret });
...
async function loadDashboardData() {
  ...
  try {
    // Load emails
    const emailsRes = await chrome.runtime.sendMessage({ action: 'getEmails' });
    const emails = emailsRes.success ? (emailsRes.data || []) : [];
    unreadCount.textContent = emails.length;
    
    // Load events
    const eventsRes = await chrome.runtime.sendMessage({ action: 'getTodayEvents' });
...
async function handleSaveNote() {
  ...
  try {
    const result = await chrome.runtime.sendMessage({ action: 'quickNote', note });
    if (result.success) {
      noteTextarea.value = '';
...
async function sendAIRequest(message, thinkingId) {
  try {
    const response = await chrome.runtime.sendMessage({ action: 'askAI', message });
...
async function loadActionsData() {
  ...
  try {
    // Load stats
    const statsRes = await chrome.runtime.sendMessage({ action: 'getActionsStats' });
    ...
    let actionsRes;
    if (filterValue === 'all') {
      actionsRes = await chrome.runtime.sendMessage({ action: 'getActions' });
    } else {
      actionsRes = await chrome.runtime.sendMessage({ action: 'getActionsByStatus', status: filterValue });
    }
